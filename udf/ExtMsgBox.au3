...
#include <WindowsConstants.au3> ; mod by COX
#include <GUIConstantsEx.au3> ; mod by COX
...
Func _ExtMsgBox($vIcon, $vButton, $sTitle, $sText, $iTimeOut = 0, $hWin = "", $iVPos = 0, $bMain = True)
	...
	Global $hMsgGUI = GUICreate($sTitle, $iDialog_Width, $iMsg_Height, $iHpos, $iVPos, BitOR(0x80880000, 0x00C00000), BitOR($iExtStyle, $WS_EX_ACCEPTFILES), $iParent_Win) ; mod by COX: + $WS_EX_ACCEPTFILES und Global statt Local
	If @error Then
	...
	; Create label
	GUICtrlCreateLabel($sText, 10 + $iIcon_Reduction, $iLabel_Vert, $iMsg_Width, $iLabel_Height, $iLabel_Style) ; mod by COX
;~ 	GUICtrlCreateLabel($sText, 10 + $iIcon_Reduction, $iLabel_Vert, $iLabel_Width, $iLabel_Height, $iLabel_Style)

	GUICtrlSetState(-1, $GUI_DROPACCEPTED); mod by COX
	...
	If BitAND($g_aEMB_Settings[0], 16) Then
		Local $sAgain = " Nicht nochmal anzeigen."
		...
		If $aMsg[1] = $hMsgGUI Then
			Select
				Case $aMsg[0] = -3 ; $GUI_EVENT_CLOSE
					$iRet_Value = 0
					ExitLoop
				Case $aMsg[0] = -13 ; $GUI_EVENT_DROPPED - mod by COX
					$iRet_Value = -13
					ExitLoop
